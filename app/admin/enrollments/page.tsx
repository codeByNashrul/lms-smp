...
export default function EnrollStudentsPage() {
  ...
  async function load() {
    try {
      ...
      const [u, c, e] = await Promise.all([
        fetch("/api/users?role=siswa"),
        fetch("/api/classes"),
        fetch("/api/enrollments"),
      ]);

      setStudents((await u.json()).items || []);
      setClasses((await c.json()).items || []);
      setItems((await e.json()).items || []);
    } finally {
      setLoading(false);
    }
  }
  ...
  async function submit(e: any) {
    ...
    if (!res.ok)
      return setMessage(data.message || "Enrollment gagal.");

    setStudent("");
    setKelas("");
    setMessage("Enrollment berhasil.");
    load();
  }
    ...
    <div className="rounded-2xl min-h-screen p-6 space-y-8 bg-gray-50 dark:bg-gray-900 transition-colors">
      ...
      <div>
        ...
        <p className="text-gray-500 dark:text-gray-400 mt-1">
          Hubungkan siswa ke dalam kelas
        </p>
      </div>
